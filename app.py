...
def generate_with_fallback(client, primary_model, contents, config):
    """
    Memanggil Gemini API dengan mekanisme fallback aman.
    Jika model utama gagal karena 404/NOT_FOUND (model tidak tersedia untuk API key ini),
    otomatis coba model cadangan di FALLBACK_MODELS secara berurutan.
    Mengembalikan tuple: (response, model_yang_benar_benar_dipakai)
    """
    ...
    for candidate_model in candidates:
        try:
            ...
            err_text = str(e)
            last_error = e
            # Hanya lanjut ke model berikutnya jika errornya soal model tidak ditemukan/tidak didukung.
            # Error lain (misal kuota habis, format request salah) langsung dilempar ke pemanggil.
            if "404" in err_text or "NOT_FOUND" in err_text.upper() or "not found" in err_text.lower():
                continue
            else:
                raise

    # Semua kandidat gagal karena 404 -> lempar error terakhir
    raise last_error
